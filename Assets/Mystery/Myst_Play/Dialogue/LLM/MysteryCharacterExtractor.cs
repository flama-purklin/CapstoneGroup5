...
public class MysteryCharacterExtractor : MonoBehaviour
{
...
#if UNITY_EDITOR
    /// <summary>
    /// Test method for character extraction - can be triggered via Editor context menu
    /// </summary>
    [ContextMenu("Test Extraction")]
    public void TestExtraction()
    {
        // Use DelayCall to avoid Unity's "TransientArtifactProvider::IsTransientArtifact" error
        UnityEditor.EditorApplication.delayCall += () =>
        {
            Debug.Log("Starting test character extraction...");
            ...
            if (File.Exists(mysteryPath))
            {
                ...
                try
                {
                    ...
                    {
                        Debug.Log($"Extraction progress: {progress:P0}");
                    };
                    
                    OnCharactersExtracted += (count) => 
                    {
                        Debug.Log($"Extraction complete. {count} character files extracted.");
                    ...
                    // Extract characters
                    ExtractCharactersFromMystery(mystery);
                    
                    Debug.Log("Test extraction complete. Check StreamingAssets/Characters folder for results.");
                }
                catch (Exception ex)
                {
                    Debug.LogError($"Error deserializing mystery JSON: {ex.Message}");
                    Debug.LogException(ex);
                }
            }
            else
    ...
    /// </summary>
    [ContextMenu("Test Nova Extraction")]
    public void TestNovaExtraction()
    {
        // Use DelayCall to avoid Unity's "TransientArtifactProvider::IsTransientArtifact" error
        UnityEditor.EditorApplication.delayCall += () =>
        {
            TestSpecificCharacter("nova_winchester");
        };
    }
    
    /// <summary>
    /// Tests the async extraction method - can be triggered via Editor context menu
    /// </summary>
    [ContextMenu("Test Async Extraction")]
    public void TestAsyncExtraction()
    {
        // Use DelayCall to avoid Unity's "TransientArtifactProvider::IsTransientArtifact" error
        UnityEditor.EditorApplication.delayCall += async () =>
        {
            Debug.Log("Starting async test character extraction...");
            ...
            if (File.Exists(mysteryPath))
            {
                ...
                try
                {
                    ...
                    OnExtractionProgress += (progress) => 
                    {
                        Debug.Log($"Async extraction progress: {progress:P0}");
                    };
                    
                    OnCharactersExtracted += (count) => 
                    {
                        Debug.Log($"Async extraction complete. {count} character files extracted.");
                    ...
                    // Extract characters asynchronously
                    int count = await ExtractCharactersAsync(mystery);
                    
                    Debug.Log($"Async test extraction complete with {count} character files. Check StreamingAssets/Characters folder for results.");
                }
                catch (Exception ex)
                {
                    Debug.LogError($"Error in async extraction: {ex.Message}");
                    Debug.LogException(ex);
                }
            }
            else
    ...
    /// <summary>
    /// Verifies a critical character (Nova) has correct speech patterns
    /// </summary>
    [ContextMenu("Verify Nova Speech Patterns")]
    public void VerifyNovaPatterns()
    {
        UnityEditor.EditorApplication.delayCall += () =>
        {
            string novaPath = Path.Combine(_outputPath, "nova_winchester.json");
            if (File.Exists(novaPath))
            {
                string novaJson = File.ReadAllText(novaPath);
                
                // Check for distinctive speech patterns
                bool hasMateTerm = novaJson.Contains("mate");
                bool hasLuvTerm = novaJson.Contains("luv");
                bool hasExpletives = novaJson.Contains("fuck") || novaJson.Contains("bloody");
                
                Debug.Log($"Nova Character File Check:");
                ...
                {
                    Debug.Log("✅ Nova's distinctive speech patterns are preserved correctly.");
                }
                else
                {
                    Debug.LogWarning("⚠️ Nova's distinctive speech patterns may be incomplete!");
                }
            }
            else
            {
                Debug.LogError($"Nova character file not found at: {novaPath}");
            }
        };
    }
#endif
    ...
    /// Tests extraction of a specific character - useful for debugging
    /// </summary>
    private void TestSpecificCharacter(string characterId)
    {
