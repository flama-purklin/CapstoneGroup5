    ...
    public static class ManageAsset
    {
        ...
        private static object CreateAsset(JObject @params)
        {
            ...
            try
            {
                ...
                {
                    // Create a material with a rendering-pipeline-agnostic approach
                    Material mat;
                    string shaderName = properties?["shader"]?.ToString();
                    
                    // Try to find the specified shader or fallbacks for different pipelines
                    Shader shader = null;
                    
                    // If shader name is provided, try to find it
                    if (!string.IsNullOrEmpty(shaderName)) {
                        shader = Shader.Find(shaderName);
                    }
                    
                    // If no shader name provided or specified shader not found, use fallbacks
                    if (shader == null) {
                        // Try render pipeline-specific shaders in sequence
                        shader = Shader.Find("Universal Render Pipeline/Lit") ?? // URP
                                Shader.Find("HDRP/Lit") ??                      // HDRP
                                Shader.Find("Standard") ??                      // Built-in
                                Shader.Find("Unlit/Color");                     // Ultimate fallback
                    }
                    
                    // Create the material with the best available shader
                    // If no shader was found at all, try to detect the active rendering pipeline
                    if (shader == null) {
                        // Try common shaders from each pipeline to determine which one is active
                        if (Shader.Find("Universal Render Pipeline/Lit") != null) {
                            Debug.Log("[ManageAsset.CreateAsset] Detected URP, using URP/Lit shader");
                            shader = Shader.Find("Universal Render Pipeline/Lit");
                        }
                        else if (Shader.Find("HDRP/Lit") != null) {
                            Debug.Log("[ManageAsset.CreateAsset] Detected HDRP, using HDRP/Lit shader");
                            shader = Shader.Find("HDRP/Lit");
                        }
                        else if (Shader.Find("Standard") != null) {
                            Debug.Log("[ManageAsset.CreateAsset] Using Standard shader (Built-in pipeline)");
                            shader = Shader.Find("Standard");
                        }
                        else {
                            Debug.LogWarning("[ManageAsset.CreateAsset] Could not detect rendering pipeline, using basic fallback");
                            shader = Shader.Find("Unlit/Color") ?? Shader.Find("Hidden/InternalErrorShader");
                        }
                    }
         ...
         private static bool ApplyMaterialProperties(Material mat, JObject properties)
        {
             ...
             // Set shader with rendering pipeline agnostic approach
             if (properties["shader"]?.Type == JTokenType.String) {
                 string shaderName = properties["shader"].ToString();
                 
                 // Don't try to set an empty shader
                 if (!string.IsNullOrEmpty(shaderName)) {
                     Shader newShader = Shader.Find(shaderName);
                     
                     // If the specified shader isn't found, try pipeline-specific fallbacks
                     if (newShader == null) {
                         Debug.LogWarning($"[ManageAsset.ApplyMaterialProperties] Shader '{shaderName}' not found. Trying fallbacks.");
                         
                         // Use a more robust fallback system
                         if (shaderName.Contains("Lit") || shaderName.Contains("Standard") || shaderName.Contains("PBR")) {
                             // Try various lit shader options across pipelines
                             newShader = Shader.Find("Universal Render Pipeline/Lit");
                             if (newShader == null) newShader = Shader.Find("HDRP/Lit");
                             if (newShader == null) newShader = Shader.Find("Standard");
                         }
                         else if (shaderName.Contains("Unlit") || shaderName.Contains("UI")) {
                             // Try various unlit shader options
                             newShader = Shader.Find("Universal Render Pipeline/Unlit");
                             if (newShader == null) newShader = Shader.Find("HDRP/Unlit");
                             if (newShader == null) newShader = Shader.Find("Unlit/Color");
                             if (newShader == null) newShader = Shader.Find("UI/Default");
                         }
                         else if (shaderName.Contains("Transparent") || shaderName.Contains("Alpha")) {
                             // Try transparent shader options
                             newShader = Shader.Find("Universal Render Pipeline/Lit");
                             if (newShader == null) newShader = Shader.Find("HDRP/Lit");
                             if (newShader == null) newShader = Shader.Find("Transparent/Diffuse");
                             if (newShader == null) newShader = Shader.Find("Standard");
                         }
                         
                         // If still not found, use a basic fallback
                         if (newShader == null) {
                             // Try to find ANY shader that might work as a generic fallback
                             ...
                             Debug.LogWarning($"[ManageAsset.ApplyMaterialProperties] Could not find appropriate shader for '{shaderName}'. Using generic fallback shader.");
                         }
                     }
