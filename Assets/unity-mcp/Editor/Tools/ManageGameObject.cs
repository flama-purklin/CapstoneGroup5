        ...
        private static object ConvertJTokenToType(JToken token, Type targetType)
        {
             try
             {
                 ...
                 if (typeof(Material).IsAssignableFrom(targetType) && token is JObject materialProps)
                 {
                     ...
                     if (!string.IsNullOrEmpty(materialPath))
                     {
                         ...
                         if (material != null)
                         {
                             ...
                             foreach (var prop in materialProps.Properties())
                             {
                                 if (prop.Name != "path")
                                 {
                                     // Special handling for shader to be rendering pipeline agnostic
                                     if (prop.Name == "shader" && prop.Value.Type == JTokenType.String)
                                     {
                                         string shaderName = prop.Value.ToString();
                                         
                                         // Only proceed if shader name is provided
                                         if (!string.IsNullOrEmpty(shaderName)) {
                                             Shader shader = Shader.Find(shaderName);
                                             
                                             // If shader not found, try fallbacks for different pipelines
                                             if (shader == null)
                                             {
                                                 Debug.LogWarning($"[ConvertJTokenToType] Shader '{shaderName}' not found. Trying fallbacks.");
                                                 
                                                 // Comprehensive shader fallback system
                                                 if (shaderName.Contains("Lit") || shaderName.Contains("Standard") || shaderName.Contains("PBR")) {
                                                     // Try lit shaders across pipelines
                                                     shader = Shader.Find("Universal Render Pipeline/Lit");
                                                     if (shader == null) shader = Shader.Find("HDRP/Lit");
                                                     if (shader == null) shader = Shader.Find("Standard");
                                                 }
                                                 else if (shaderName.Contains("Unlit") || shaderName.Contains("UI")) {
                                                     // Try unlit shaders across pipelines
                                                 ...
                                                 if (shader == null) {
                                                     ...
                                                     Debug.LogWarning($"[ConvertJTokenToType] Using fallback shader for '{shaderName}'.");
                                                 }
                                             }
                                             
                                             // Only set shader if we found a valid one
                                             if (shader != null) {
                     ...
                     // If path not specified, could be creating a new material
                     if (materialProps["new"] != null && materialProps["new"].ToObject<bool>())
                     {
                         // Create a new material with rendering pipeline agnostic approach
                         string shaderName = materialProps["shader"]?.ToString();
                         Shader shader = null;
                         ...
                         if (!string.IsNullOrEmpty(shaderName))
                         {
                             ...
                             // Try fallbacks if shader not found
                             if (shader == null)
                             {
                                 Debug.LogWarning($"[ConvertJTokenToType] Shader '{shaderName}' not found for new material. Trying fallbacks.");
                         ...
                         // If no shader specified or nothing found, use a sequence of fallbacks for the active pipeline
                         if (shader == null) {
                             // Try to detect the active rendering pipeline
                             // First check if any of the pipeline-specific shaders exist
                             shader = Shader.Find("Universal Render Pipeline/Lit");
                             if (shader != null) {
                                 Debug.Log("[ConvertJTokenToType] Detected URP, using URP/Lit shader for new material");
                             } else {
                                 shader = Shader.Find("HDRP/Lit");
                                 if (shader != null) {
                                     Debug.Log("[ConvertJTokenToType] Detected HDRP, using HDRP/Lit shader for new material");
                                 } else {
                                     shader = Shader.Find("Standard");
                                     if (shader != null) {
                                         Debug.Log("[ConvertJTokenToType] Using Standard shader for new material");
                                     } else {
                                         shader = Shader.Find("Unlit/Color");
                                         Debug.LogWarning("[ConvertJTokenToType] Fallback to Unlit/Color for new material");
                                     }
                                 }
                             }
                         }
                         
                         // Create material with best available shader or fallback to error shader
                         Material newMaterial = new Material(shader != null ? shader : Shader.Find("Hidden/InternalErrorShader"));
                         
                         // Apply other properties
                         foreach (var prop in materialProps.Properties())
                         {
                             if (prop.Name != "path" && prop.Name != "new" && prop.Name != "shader")
                             {
                                 SetProperty(newMaterial, prop.Name, prop.Value);
                             }
                         }
                     ...
                     // If no path is specified and not creating new, could be a dynamic material or instance set by reference                     
                     return null;
                 }
